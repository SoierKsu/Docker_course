...
def parse_cyclonedx(sbom_file):
    ...
    # Извлечение компонентов из XML
    components = []
    for component in root.findall('.//{http://cyclonedx.org/schema/bom/1.4}component'):
        component_data = {}
        component_data['bom-ref'] = component.get('bom-ref')
        component_data['name'] = component.findtext('{http://cyclonedx.org/schema/bom/1.4}name')
        
        dependencies = []
        for dep in component.findall('.//{http://cyclonedx.org/schema/bom/1.4}dependency'):
            ref = dep.get('ref')
            if ref:
                dependencies.append(ref)
        component_data['dependsOn'] = dependencies
...
if __name__ == "__main__":
    sbom_file = "path_to_your_sbom.xml"  # Замените на путь к вашему SBOM файлу
    try:
